      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 flex flex-col flex-1 overflow-hidden">
        ...
        <div className="px-6 py-5 border-b border-gray-200 flex justify-between items-center bg-white sticky top-0 z-10">
           <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
              <Users className="w-5 h-5 text-red-600" /> Citizen Directory <span className="text-gray-400 font-normal">(Public Grievance Profiles)</span>
           </h2>
           <div className="flex items-center gap-3">
             <div className="text-right hidden sm:block">
               <p className="text-xs font-bold text-gray-400 uppercase tracking-tighter">Total Registered</p>
               <p className="text-sm font-bold text-gray-900">{citizens.length}</p>
             </div>
             <button className="bg-red-600 hover:bg-black text-white font-bold px-4 py-2 rounded-lg text-sm transition-all shadow-md">
               Export CSV
             </button>
           </div>
        </div>
        ...
        <div className="px-6 py-3 border-b border-gray-100 bg-gray-50/50 flex">
           <div className="relative w-96">
             ...
             <input 
               ...
               className="pl-9 pr-4 py-2 w-full border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 bg-white transition-all shadow-sm" 
             />
           </div>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead className="sticky top-0 bg-gray-50 shadow-sm z-10">
              <tr className="text-xs font-bold text-gray-500 border-b border-gray-200">
                <th className="p-4 pl-6 uppercase tracking-wider">Citizen Identity</th>
                <th className="p-4 uppercase tracking-wider">Phone / WhatsApp</th>
                <th className="p-4 uppercase tracking-wider">Reports</th>
                <th className="p-4 uppercase tracking-wider">Registration Area</th>
                <th className="p-4 text-right pr-6 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody className="bg-white">
              {loading ? (
                <tr><td colSpan={5} className="p-12 text-center text-gray-400 font-bold uppercase text-xs animate-pulse">Synchronizing Citizen Data...</td></tr>
              ) : filteredCitizens.length === 0 ? (
                <tr><td colSpan={5} className="p-12 text-center text-gray-500">No citizens match your search parameters.</td></tr>
              ) : (
                filteredCitizens.map((cz) => {
                  const comps = complaintsByUser[cz.phone] || [];
                  const displayAddress = cz.address || 'Unknown Location';

                  return (
                    <tr 
                      key={cz.phone} 
                      className="border-b border-gray-100 hover:bg-red-50/30 transition-colors cursor-pointer group"
                      onClick={() => setSelectedCitizen({ ...cz, address: displayAddress, complaints: comps })}
                    >
                      <td className="p-4 pl-6">
                        <div className="flex items-center gap-3">
                          <img 
                            src={`https://ui-avatars.com/api/?name=${cz.name || 'Citizen'}&background=random&color=fff&bold=true`} 
                            alt="avatar" 
                            className="w-10 h-10 rounded-full bg-gray-200 border-2 border-white shadow-sm ring-1 ring-gray-100" 
                          />
                          <div className="flex flex-col">
                            <span className="text-sm font-bold text-gray-900 group-hover:text-red-700 transition-colors">{cz.name || 'Anonymous Citizen'}</span>
                            <span className="text-[10px] uppercase font-bold text-gray-400 tracking-wider">WARD: {cz.ward || 'N/A'}</span>
                          </div>
                        </div>
                      </td>
                      <td className="p-4">
                        <div className="flex items-center gap-1.5">
                           ...
                           <span className="text-sm font-mono text-gray-600">+{cz.phone}</span>
                        </div>
                      </td>
                      <td className="p-4">
                         <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${comps.length > 0 ? 'bg-red-100 text-red-700 border border-red-200' : 'bg-gray-100 text-gray-500'}`}>
                            {comps.length} {comps.length === 1 ? 'Report' : 'Reports'}
                         </span>
                      </td>
                      <td className="p-4 text-sm text-gray-500 truncate max-w-[250px]">{displayAddress}</td>
                      <td className="p-4 text-right pr-6 text-gray-400">
                         <button className="p-2 hover:bg-white rounded-lg transition-colors border border-transparent hover:border-gray-100 hover:shadow-sm">
             ...
             <div className="p-4 sm:p-8 flex flex-col gap-6 sm:gap-8 overflow-y-auto max-h-[70vh] custom-scrollbar">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-10">
                   ...
                   <div className="space-y-6">
                      <div>
                        <h3 className="text-[10px] font-bold uppercase text-gray-400 tracking-widest mb-4 flex items-center gap-2"><MapPin className="w-3.5 h-3.5"/> Primary Address</h3>
                        <div className="bg-red-50/50 p-4 rounded-xl border border-red-100 border-l-4 border-l-red-600">
                           <p className="text-sm text-gray-900 font-bold leading-relaxed">{selectedCitizen.address}</p>
                           <p className="text-[10px] font-bold text-red-600 mt-2 uppercase tracking-tighter">WARD: {selectedCitizen.ward || 'N/A'}</p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                         <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                            ...
                            <span className="text-xl font-bold text-gray-900">{selectedCitizen.complaints?.length || 0}</span>
                         </div>
                         <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                            <span className="text-[10px] font-bold text-gray-400 uppercase block mb-1">Pincode</span>
                            <span className="text-xl font-bold text-gray-900">{selectedCitizen.pincode || 'N/A'}</span>
                         </div>
                      </div>
                   </div>
                   ...
                   {/* Right: History Feed */}
                   <div>
                     <h3 className="text-[10px] font-bold uppercase text-gray-400 tracking-widest mb-4 flex items-center gap-2"><Calendar className="w-3.5 h-3.5"/> Recent Engagement</h3>
                     <div className="space-y-4 max-h-[220px] overflow-y-auto pr-2 custom-scrollbar">
                       {selectedCitizen.complaints?.length > 0 ? (
                         selectedCitizen.complaints.map((cp: any, i: number) => (
                           <div key={i} className="group bg-white p-4 rounded-xl border border-gray-100 hover:border-red-200 hover:shadow-md transition-all cursor-default">
                              <div className="flex justify-between items-start mb-1">
                                 <p className="text-xs font-bold text-gray-800">{cp.complaintId || cp.id.substring(1,8)}</p>
                                 <span className="text-[10px] font-bold text-red-600 bg-red-50 px-1.5 py-0.5 rounded">{cp.status.toUpperCase()}</span>
                              </div>
                              <p className="text-[11px] text-gray-500 font-medium truncate">{cp.category}: {cp.description}</p>
                           </div>
                         ))
                       ) : (
                          <div className="py-10 text-center flex flex-col items-center gap-2">
                             ...
                             <p className="text-sm text-gray-400 font-bold">No historical data available</p>
                          </div>
                       )}
                     </div>
                   </div>
          ...
          <div className="px-8 py-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
             <div>
               ...
               <p className="text-[10px] font-bold text-gray-400 mt-0.5 uppercase tracking-widest">
                 {selectedCitizen?.name || 'Citizen'} • +{selectedCitizen?.phone}
               </p>
             </div>
             <button 
